...
func testBookToManyLikes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := BookSlice{&a}
	if err = a.L.LoadLikes(ctx, tx, false, (*[]*Book)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testBookToManyAddOpLikes(t *testing.T) {
	var err error
...
var (
	bookDBTypes = map[string]string{`ID`: `integer`, `Title`: `character varying`, `Genre`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
